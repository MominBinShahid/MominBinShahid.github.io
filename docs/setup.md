...
Just make sure **node >=16.0.0** is installed, see package.json for more details

> **Apple Silicon Mac Users:** Run `npm run install:apple-silicon` instead of `npm run init`. See `docs/notes.md` for details.
...
- Before running any command make sure to use the correct Node version (16.x):
  - **nvm:** `nvm use` (reads from `.nvmrc`)
  - **fnm:** `fnm use` (reads from `.node-version`)
  - **volta:** Automatic (reads from `package.json` volta config)
